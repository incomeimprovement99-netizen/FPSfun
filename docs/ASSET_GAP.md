...
|---|---|---|---|
| **Characters** | One motion-captured mannequin (Quaternius, CC0) and one robot built in code. Five operators and ten outfits, in three builds, any of which a player can put on their own loadout: a colour scheme, clothes measured to the body they hang on, a choice of face covering and ten pieces of kit. | ~25 hand-modelled legends, each a unique silhouette, each with dozens of skins, each with bespoke facial rig and voice. | Ours now read as **different people**, which they did not this morning. They still read as *one body in different clothes*: same height, same build, same face. |
...
|---|---|---|---|
| 1 | ~~**More outfits, and let a player choose one**~~ **Done (Milestone 159).** | Ten sets now, a picker on the Loadouts tab (outfit, build, face), kept per loadout, and it crosses the wire so a friend sees it. | done |
| 2 | **The torso, measured like the limbs were** | The limbs now follow a profile measured off the model; the body is still one box tuned by eye. The spine bone leans back, so the box leans with it and the tops of the shoulders stand outside it: that is the pale patch at the collar in `outfit-close`. Same fix as the limbs, one dimension harder: a centre and a half-depth per band up the bone, and a garment built to follow it. `tools/checks/body.ts` already prints the numbers it would need. | half a day |
| 3 | **Body variety: a second head shape, and a sleeveless set** | Mostly done. The three **builds** shipped with the picker and change the clothes rather than the body on purpose: hit boxes must not move. A hood, a ghillie's rags and a tracksuit stripe shipped with them, which took the wardrobe from two silhouettes to five. What is left is what clothes cannot do: every head is the same head, and nothing in the wardrobe bares an arm the way a plate carrier over a t-shirt would. | half a day |
| 4 | **A rifle animation set** | The library is a pistol library, which is why long guns are posed by IK rather than animated. Quaternius's newer packs and Mixamo's CC0 set have rifle idles, walks and reloads. Retargeting one set onto this rig would make every figure holding a long gun read as holding it rather than presenting it. | 1-2 days |
| 5 | **Weapon surface detail** | The guns are good geometry with flat materials. A single shared "gunmetal" PBR set with wear in the roughness map, plus a decal per manufacturer, would lift all thirty at once. | 1 day |
| 6 | **Recorded gunshots, chosen by ear** | Half-built: the mechanical layer is recorded, the report is synthesised. A CC0 library (Airborne Sound's free set, gamesounds.xyz) picked per class and layered under the synthesis is the last big audio gap. **Needs the owner to pick by ear**, which is why it has stalled. | 1 day + listening |
| 7 | **Voice: eight lines per operator** | No voice at all. Eight lines (ping, enemy, thanks, knocked, revived, kill, last squad, victory) from a CC0 pack or a local TTS pass would be the single biggest "shipped game" signal after characters. | 1 day |
| 8 | **The range stops being boxes** | The arenas and the BR map now wear props; the range, which is the first thing anybody sees, is still mostly boxes with decals on them. Same dressing pass, same wardrobe. | half a day |
| 9 | **Decals and wear** | No scuffs, no leaks, no signage beyond the graffiti. A dozen CC0 decals placed by the same rules the props use would do more for "somebody built this" than another prop set. | 1 day |
| 10 | **A second battle royale map** | Still the biggest single item on the whole roadmap, and still 3-4 days. It is here rather than at the top because everything above it is cheaper per hour of player-visible change. | 3-4 days |
| 11 | **Per-surface footsteps** | One footstep sound for concrete, sand, metal and wood. The material is already known at the impact point, so this is a lookup and a sound set. | half a day |
...
for less visible change than items 1 to 9 together.
