...
| 2 | ~~**The torso, measured like the limbs were**~~ **Done (Milestone 160).** | Six bands up the spine, each with the body's width, depth and middle, measured off the model; the jacket is a tube of rectangular sections built on them instead of one box, and `fit.torso`/`fit.torsoAt` are gone. The kit in `gear.ts` has its own numbers and was never on that box; measuring it against these bands is the next of these. | done |
