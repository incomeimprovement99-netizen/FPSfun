...
}
{
  ...
  // what grows on the sand (br.ts flora): drawn by cell and distance like the rocks, and the trees, the dear part, held to dozens
  const flora = sc.flora;
  const trees = flora.filter((f) => f.kind.startsWith("tree")).length;
  check("the field has something growing on it, and the trees stay in the dozens", flora.length > 300 && flora.length < 1500 && trees > 20 && trees < 80, `${flora.length} in all, ${trees} trees`);
