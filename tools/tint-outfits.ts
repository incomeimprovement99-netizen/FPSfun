...
/** the atlas a set's parts are painted with, from the parts' own names */
function atlasFor(parts: string[]): string | null {
  const set = parts.some((p) => p.includes("Ranger")) ? "Ranger" : parts.some((p) => p.includes("Peasant")) ? "Peasant" : null;
  return set ? `T_${set}_BaseColor.webp` : null;
}

async function main(): Promise<void> {
  mkdirSync(TINTS, { recursive: true });
  const sets = outfitCfg.sets as Record<string, { parts?: string[]; tint?: string }>;
  let made = 0;
  for (const [id, set] of Object.entries(sets)) {
    if (!set.tint || !set.parts?.length) continue;
    const atlas = atlasFor(set.parts);
    if (!atlas || !existsSync(join(OUT, atlas))) continue;
    ...
    await sharp(join(OUT, atlas))
      ...
      .linear([(rgb.r / 255) * 1.35, (rgb.g / 255) * 1.35, (rgb.b / 255) * 1.35], [0, 0, 0])
      .webp({ quality: 82 })
      .toFile(join(TINTS, `${id}.webp`));
    made++;
  }
  const size = readdirSync(TINTS).reduce((n, f) => n + (f.endsWith(".webp") ? 1 : 0), 0);
  console.log(`${made} outfit atlases written to public/models/outfits/tints (${size} files)`);
}
